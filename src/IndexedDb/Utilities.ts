...
export const saveApplicationContext = async (settings: PomodoroSettings, db: IDBPDatabase<pomodoroDb>) => {
    try {
        const transaction = db.transaction("pomodoroSettings", "readwrite");
        const store = transaction.objectStore("pomodoroSettings");
        await store.put(
            settings
            , userPomodoroSettingsKey)
...
export const getSavedApplicationContext = async (db: IDBPDatabase<pomodoroDb>) => {
    try {
        const transaction = db.transaction("pomodoroSettings", "readwrite");
        const store = transaction.objectStore("pomodoroSettings");
